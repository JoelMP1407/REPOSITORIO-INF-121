...
public class JuegoAdivinaImpar extends JuegoAdivinaNumero{
    public JuegoAdivinaImpar(int numDeVidas, int numeroAAdivinar) {
    ...
    public boolean validaNumero(int numero) {
        if (numero >= 0 && numero <= 10) {
            if(numero % 2 != 0){
                ...
                System.out.println("Error: el número ingresado es par. Solo se permiten numeros impares entre 0 y 10.");
                return false;
            }
        } else {
    ...
    numeroAAdivinar = 1+2*random.nextInt(5);
        while (true) {
